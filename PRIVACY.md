# Privacy Policy — TidyUp

**Last updated: 22 September 2026 (version 1.0.0).**

TidyUp is a file organiser for Windows. It watches folders you choose and moves new files
...
**TidyUp has no network code. It cannot send anything anywhere.** There are no accounts, no
...
- **The website a download came from.** Windows attaches a small hidden note (the
  ...
  from. TidyUp reads the site name from that note so a rule like "files from moodle go to
...
TidyUp never opens or reads the contents of your files.

## What TidyUp writes

All of it lives in `%APPDATA%\com.arpon.tidy\` on your computer:
...
Delete that folder and TidyUp forgets everything. Uninstalling the app leaves your files and
the category folders exactly where they are; nothing of yours is removed.

## What TidyUp never does
...
TidyUp itself still has no network code. Start-with-Windows in the Store version is managed
by Windows (Settings > Apps > Startup).

## Questions

Open an issue at <https://github.com/nur-arpon/TidyUp/issues>.
